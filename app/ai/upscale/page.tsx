...
export default function UpscalePage() {
  ...
  const handleUpscale = async () => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/tools/upscale`, {
        method: 'POST',
        headers: {
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="px-4 py-12 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-4xl">
          <div className="mb-8">
            <h1 className="text-4xl font-bold mb-2">Upscale Image</h1>
            <p className="text-muted-foreground">Increase resolution up to 4x without quality loss</p>
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            ...
            <div className="space-y-6">
              ...
              <div>
                <label className="block text-sm font-semibold mb-2">Upscale Factor</label>
                <select
                  value={scale}
                  onChange={(e) => setScale(e.target.value)}
                  className="w-full p-3 bg-secondary border border-border rounded-lg focus:outline-none"
                >
                  <option value="2x">2x (2x upscale)</option>
                  <option value="4x">4x (4x upscale)</option>
                </select>
              </div>
              ...
              >
                {loading ? 'Upscaling...' : <>Upscale Image <Zap className="h-4 w-4" /></>}
              </Button>
            </div>
              ...
              <div className="glass rounded-lg p-6 min-h-96 flex items-center justify-center">
                {loading ? (
                  <div className="text-center">
                    ...
                    <p className="text-muted-foreground">Upscaling...</p>
                  </div>
